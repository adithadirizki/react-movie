...
import { getPopularMovie } from "../actions";
import ErrorMovie from "./utils/Error";
import { useDispatch } from "react-redux";
import { SET_SELECTED_MENU } from "../actions/types";

const PopularMovie = ({ popularMovie, getPopularMovie }) => {
  const { page, total_pages } = popularMovie.data;
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch({ type: SET_SELECTED_MENU, payload: "Popular" });
    getPopularMovie();
  }, []);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {popularMovie.data.results.map((value, index) => {
          return <MovieItem movie={value} key={index} />;
        })}
        {popularMovie.loading ? <SkeletonPreload /> : null}
      </div>
...
const mapStateToProps = ({ popularMovie }) => {
  return { popularMovie };
};

export default connect(mapStateToProps, { getPopularMovie })(PopularMovie);
