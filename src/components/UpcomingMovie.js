...
import { getUpcomingMovie } from "../actions";
import ErrorMovie from "./utils/Error";
import { useDispatch } from "react-redux";
import { SET_SELECTED_MENU } from "../actions/types";

const UpcomingMovie = ({ upcomingMovie, getUpcomingMovie }) => {
  const { page, total_pages } = upcomingMovie.data;
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch({ type: SET_SELECTED_MENU, payload: "Upcoming" });
    getUpcomingMovie();
  }, []);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {upcomingMovie.data.results.map((value, index) => {
          return <MovieItem movie={value} key={index} />;
        })}
        {upcomingMovie.loading ? <SkeletonPreload /> : null}
      </div>
...
export default connect(mapStateToProps, { getUpcomingMovie })(UpcomingMovie);
